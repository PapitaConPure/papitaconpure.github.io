	...
	return (
		<button
			...
			onClick={() =>
				isPlayingThisTrack
					? stopAudioPlayer()
					: sendTrackToAudioPlayer(targetTrack)
			}
...
export function AssetDownloadsBrowser({
	item,
	lang,
	t,
	...props
}: AssetDownloadsBrowserProps) {
	const [activeKindFilters, setActiveKindFilters] = useState(
		() => new Set<AssetKind>(),
	);
	const [activeFormatFilters, setActiveFormatFilters] = useState(
		() => new Set<AssetFormat>(),
	);
	const [maxDisplayedAssets, setMaxDisplayedAssets] = useState(
		MAX_DISPLAYED_ASSETS_DEFAULT,
	);
	...
	function isDownloadFormatDisplayed(
		downloadFormat: AssetFormat | null = null,
	) {
		if (activeKindFilters.size === 0) return true;
